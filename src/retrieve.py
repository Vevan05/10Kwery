...
if not OPENAI_API_KEY:
    raise RuntimeError(
        "OPENAI_API_KEY not set. Add it to your .env file, e.g.\n"
        '  OPENAI_API_KEY="sk-..."'
...
def truncate_for_embedding(text: str) -> str:
    ...
    return ENCODING.decode(tokens[:MAX_EMBED_TOKENS])
...
def build_vector_index(chunks: list[dict]) -> None:
    ...
    collection = chroma_client.get_or_create_collection(
        name = "filings", metadata = {"hnsw:space": "cosine"}
    )
    ...
    for i in tqdm(range(0, len(chunks), EMBED_BATCH_SIZE), desc = "Embedding batches"):
        batch = [c for c in chunks[i : i + EMBED_BATCH_SIZE] if c["chunk_id"] not in existing_ids]

        if not batch:
            continue

        texts = [truncate_for_embedding(c["text"]) for c in batch]
